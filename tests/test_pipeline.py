...
def test_input_impersonating_a_sanitizer_report_is_not_a_crash():
    """The input is generated by us, so it can contain anything -- including
    the exact strings crash detection looks for.

    mxml quotes the offending element name back in its error message, so a
    document named <AddressSanitizer> makes a clean rejection look like an
    ASan report. Hypothesis mines string constants from local modules and
    feeds them into generated text, so it injects these very strings; this
    fired in a real 40-example run.
    """
    real = Path(__file__).resolve().parents[1] / "build" / "harness"
    if not real.exists():
        pytest.skip("run `make` first")

    for document in (
        b"<AddressSanitizer></b>",
        b'<a x="runtime error:"></b>',
        b"<UndefinedBehaviorSanitizer></z>",
    ):
        result = run_once(real, document)
        assert result.outcome is Outcome.REJECTED, (
            f"{document!r} is a clean rejection, not a crash; got "
            f"{result.outcome.value} from stderr {result.stderr[:120]!r}"
        )
